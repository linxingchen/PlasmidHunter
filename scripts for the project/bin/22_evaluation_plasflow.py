...
def evaluate(input_contigs, predict_out):
    ...
    df = pd.read_csv(predict_out, sep='\t', header=0, index_col=2)
    y_pred = df.loc[:,'label'].to_dict()
    ...
    y_pred = [0 if i.split('.')[0]=='chromosome' else 1 if i.split('.')[0]=='plasmid' else 2 for i in y_pred]
